    ...
    void descriptions(Route &route, IRouter* router, QStringList* icons, QVector< IRouter::Node > pathNodes, QVector< IRouter::Edge > pathEdges, int units, int maxSeconds = INT_MAX)
    {
        ...
        for (int edge = 0; edge < pathEdges.size() - 1; edge++)
        {
            ...
            if(breakDescription)
            {
                ...
                QStringList labels;
                QString nextName;
                // UNUSED bool nameAvailable = router->GetName(&nextName, pathEdges[edge + 1].name);
        ...
        // if(seconds < maxSeconds)
        {
            ...
            // UNUSED bool nameAvailable = router->GetName(&nextName, pathEdges.last().name);
            // UNUSED bool typeAvailable = router->GetType(&nextType, pathEdges.last().type);
