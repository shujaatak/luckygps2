...
spatialite_datasource::spatialite_datasource(parameters const& params)
   : datasource(params),
     extent_(),
...
std::string const spatialite_datasource::name_="spatialite";
...
Envelope<double> spatialite_datasource::envelope() const
{
   return extent_;
}
...
featureset_ptr spatialite_datasource::features(query const& q) const
{
   if (dataset_)
   {
		...
		return featureset_ptr (new spatialite_featureset(rs, desc_.get_encoding(), format_, multiple_geometries_));
   }
